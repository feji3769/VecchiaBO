...
//[[Rcpp::export]]
torch::Tensor MaxMincpp(torch::Tensor locations)
{
  unsigned int N = (unsigned int)locations.size(0);
  int dim = int(locations.size(1));
  torch::Tensor res = torch::ones(N, torch::TensorOptions().dtype(torch::kInt32));
  //IntegerVector res(N);
  ...
  for (int i = 0; i < (int)N; i++)
  {
    for(int j = 0; j < dim; j++)
    {
      average_arr[j] += (coords[dim * i + j] = locations.index({i,j}).item<double>());
    }
  }
  for(int j = 0; j < dim; j++)
  {
  ...
  //res[0] = int(first_node + 1);
  res.index_put_({0}, int(first_node + 1));
  for (int i = 1; i < (int)N; i++)
    res.index_put_({i}, int(P[i] + 1));
